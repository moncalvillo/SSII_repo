...
public class ServerController {
    ...
    public ResponseEntity getFiles(HttpServletRequest req, HttpServletResponse resp) {
        var f = serverService.getFiles();
        return new ResponseEntity<>(f, HttpStatus.OK);
    }

    @RequestMapping(value = "/populate", method = RequestMethod.POST)
    public ResponseEntity populateDatabase(HttpServletRequest req) throws IOException {
        String directory = req.getParameter("directory");

        serverService.populateDatabase(directory);
